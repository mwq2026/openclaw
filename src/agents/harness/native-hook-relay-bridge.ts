...
export function createNativeHookRelayBridgeRuntime(context: {
  relays: Map<string, ActiveNativeHookRelayRegistration>;
  relayBridges: Map<string, NativeHookRelayBridgeRegistration>;
  invokeNativeHookRelay: (
    params: InvokeNativeHookRelayParams & { requireGeneration?: boolean },
  ) => Promise<NativeHookRelayProcessResponse>;
  log: Pick<ReturnType<typeof createSubsystemLogger>, "debug">;
}) {
  const { relays, relayBridges, invokeNativeHookRelay, log } = context;
  async function invokeNativeHookRelayBridge(
  ...
  function isNativeHookRelayBridgeStaleRegistrationError(error: unknown): boolean {
    return (
